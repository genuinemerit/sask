...
- **`docs/help_src/getting-started.md`**'s "Getting data as JSON" section
  now points to `/api/reference` (HTML) and `/api/reference?format=json`
  (JSON) as the complete reply-format reference, noting it's English-only.
  Rebuilt via `build_i18n_pages.py` (en-US deterministic); the es-ES
  translation was hand-updated to match and re-acknowledged via
  `check_page_staleness.py --acknowledge getting-started`.
- **`tools/dev/build_api_reference.py`**: removed the "Generated by
  tools/dev/build_api_reference.py..." meta line from the HTML and the
  `generated_by` key from the JSON — build provenance is dev/AI-audience
  chrome, not useful to the reference's actual reader. Rebuilt; both
  artifacts and the staleness check stay clean.

Full suite: 1067, all passing. Pre-commit clean.
